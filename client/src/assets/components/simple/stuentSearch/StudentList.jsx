...
export default function StudentList(props) {
    ...
    let students = props.studentList;
    ...
    function setDataEdit(id, name, email, phone, series, userClass, firstName, lastName, birthDate) {
        setStudentNameBeDeleted(name.toUpperCase());
        setStudentIdBeDeleted(id);
        setEmail(email);
        setPhone(phone);
    ...
    function deleteStudent(id) {
        ...
        fetch('/user/removeStudent', options).then(res => {
            ...
            toast.success('Aluno removido com sucesso', students);
        });
    ...
    function editStudent() {
        ...
        fetch('/user/updateStudent', options).then(res => {
            if (res.status === 400) {
                    ...
                    <li className='list-group-item d-flex justify-content-center align-items-center' key={item._id} style={{ width: "100%" }}>
                        <div className='d-flex flex-column  card bg-light mb-3' style={{ width: "100%" }}>
                            <div className='card-header d-flex flex-row justify-content-between align-items-center' style={{ width: "100%" }}>
                                <button onClick={() => { setDataEdit(item._id, item.fullName, item.email, item.phone, item.series, item.userClass, item.firstName, item.lastName, item.birthDate) }} data-bs-toggle='modal' data-bs-target='#setMensageModal' className='card-title fs-5 btn'>{item.fullName}</button>
                                <div className=' d-flex flex-column flex-sm-row flex-wrap bg-light p-1 justify-content-end align-items-center'>

                                    <button data-bs-toggle='modal' data-bs-target='#editDeletStudentModal' className='btn btn-primary m-1' title='editar' onClick={() => { setDataEdit(item._id, item.fullName, item.email, item.phone, item.series, item.userClass, item.firstName, item.lastName, item.birthDate) }}>
                ...
                <div className="alert alert-secondary modal-dialog">
                    <div className="modal-content">
                        ...
                        <div className="modal-body">
                            ...
                            <input className="form-control mb-1" placeholder='Primeiro nome' type="text" value={firstName} onChange={e => setFirstName(e.target.value)} />
                            <input className="form-control mb-1" placeholder='Sobrenome' type="text" value={lastName} onChange={e => setLastName(e.target.value)} />
                            <input className="form-control mb-1" placeholder='Celular' type="text" value={phone} onChange={e => setPhone(e.target.value)} />
                            <div className='input-group mb-1'>
                                <select className='form-select' id="inputGroupSelect01" value={series} onChange={e => setSeries(e.target.value)}>
                            ...
                            <div className='input-group mb-1'>
                                <select className='form-select' id="inputGroupSelect02" value={userClass} onChange={e => setUserClass(e.target.value)}>
                            ...
                            <input className="form-control mb-1" placeholder='Email' type="text" value={email} onChange={e => setEmail(e.target.value)} />
                            <input className="form-control mb-1" placeholder='Senha' type="password" value={password} onChange={e => setPassword(e.target.value)} />
                            <input className="form-control mb-1" placeholder='Data de nascimento' type="date" value={birthDate} onChange={e => setBirthDate(e.target.value)} />

                        </div>
                        <div className="modal-footer">
                            <button type="button" className="btn btn-danger" data-bs-dismiss="modal">Cancelar</button>
                            <button onClick={() => { editStudent() }} type="button" className="btn btn-success" data-bs-dismiss="modal">Confirmar</button>
                        </div>
                    </div>
                </div>
            </div>
